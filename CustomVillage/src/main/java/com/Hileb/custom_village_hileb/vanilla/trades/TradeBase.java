...
public class TradeBase {
    public static class Loaded extends LoadedVillage {
        ...
        public void run(IForgeRegistry<VillagerRegistry.VillagerProfession> r) {
            ...
            try{
                for(VillagerRegistry.VillagerCareer career1: (List<VillagerRegistry.VillagerCareer>)(ReflectionHandler.villagerRegistry_villagerProfession$careers.get(profession_v))){
        ...
        if (json.has("nbt"))
        {
            ...
            try
            {
                ...
                if(element.isJsonObject())
                    nbt = JsonToNBT.getTagFromJson(CustomVillageLoader.GSON.toJson(element));
                else
